...
Functions without upvalues stay as plain ObjFunction values and are only wrapped
in ObjClosures when they actually close over variables. This removes the extra
allocation and indirection from the common no-upvalue case while preserving the
closure path for functions that need captured state.
*/
