...
Functions without upvalues are executed directly, so the VM stores the raw
ObjFunction in each call frame and only keeps an ObjClosure when one is needed
for captured upvalues. That removes the common-case closure allocation while
preserving the existing closure behavior for nested functions that capture
locals.
*/
...
InterpretResult interpret(const char* source) {
    ...
    if (!callFunction(function, NULL, 0)) {
        return INTERPRET_RUNTIME_ERROR;
    }
    return run();
