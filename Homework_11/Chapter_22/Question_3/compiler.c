...
Adds support for const declarations and compile-time assignment checks so
single-assignment variables cannot be reassigned.
*/
...
#define GLOBAL_DECL_MAX 1024

typedef struct {
    Local locals[UINT8_COUNT];
    GlobalDecl globals[GLOBAL_DECL_MAX];
    int globalCount;
    int localCount;
    int scopeDepth;
} Compiler;
...
static void declareGlobal(Token name, bool isMutable) {
    if (current->globalCount == GLOBAL_DECL_MAX) {
        error("Too many global declarations in one chunk.");
        return;
    }

    GlobalDecl* global = &current->globals[current->globalCount++];
    global->name = name;
...
static void namedVariable(Token name, bool canAssign) {
    ...
    bool targetIsMutable = true;
    if (arg != -1) {
        getOp = OP_GET_LOCAL;
        setOp = OP_SET_LOCAL;
        targetIsMutable = current->locals[arg].isMutable;
    } else {
        arg = identifierConstant(&name);
        getOp = OP_GET_GLOBAL;
        setOp = OP_SET_GLOBAL;

        int global = resolveGlobal(&name);
        if (global != -1) {
            targetIsMutable = current->globals[global].isMutable;
        }
    }
