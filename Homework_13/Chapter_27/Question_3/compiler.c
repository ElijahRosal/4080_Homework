...
Adds deleteStatement() parser function and parse rule to handle delete statements
for both direct (delete instance.field) and computed (delete instance[expr]) field deletion.
*/
...
static void deleteStatement() {
    ...
    if (match(TOKEN_DOT)) {
        ...
        expression();
        consume(TOKEN_RIGHT_BRACKET, "Expect ']' after index expression.");
        emitByte(OP_DELETE_INDEX);
    } else {
        error("Can only delete properties.");
    }
