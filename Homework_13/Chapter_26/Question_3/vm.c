...
Converts the VM to use retain/release ownership, including stack movement,
upvalue closure, and call-frame cleanup.
*/
    ...
    if (IS_OBJ(callee)) {
        switch (OBJ_TYPE(callee)) {
            ...
            case OBJ_NATIVE: {
                ...
                Value* stackBase = vm.stackTop - argCount - 1;
                Value result = native(argCount, vm.stackTop -
               argCount);
                retainValue(result);
                while (vm.stackTop > stackBase) {
                    pop();
                }
                push(result);
                releaseValue(result);
...
static ObjUpvalue* captureUpvalue(Value* local) {
    ...
    retainObject((Obj*)createdUpvalue);
        ...
        switch (instruction = READ_BYTE()) {
            ...
            case OP_CLOSURE: {
                ...
                for (int i = 0; i < closure->upvalueCount; i++) {
                    ...
                    retainObject((Obj*)closure->upvalues[i]);
                }
                break;
            }
